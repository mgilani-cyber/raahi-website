...
export const FullScreenScrollFX = forwardRef<HTMLDivElement,FullScreenFXProps>(({
  ...
  header, footer, gap=0, gridPaddingX=3,
  showProgress=true, debug=false,
  durations={change:0.7,snap:800}, reduceMotion,
  bgTransition="fade", parallaxAmount=4,
  currentIndex, onIndexChange, initialIndex=0,
  colors={text:"rgba(232,224,204,0.92)",overlay:"rgba(8,25,16,0.6)",pageBg:"#081910",stageBg:"#081910"},
  apiRef, ariaLabel="Menu",
}, ref) => {
  ...
  const computePositions=()=>{
    ...
    if(!el)return;
    const top=el.offsetTop,h=el.offsetHeight,arr:number[]=[];
  ...
  const cssVars:CSSProperties={
    ...
    ["--fx-text"as any]:colors.text??"rgba(232,224,204,0.92)",
    ["--fx-overlay"as any]:colors.overlay??"rgba(8,25,16,0.6)",
    ["--fx-page-bg"as any]:colors.pageBg??"#081910",
    ["--fx-stage-bg"as any]:colors.stageBg??"#081910",
  ...
  const fxStyles = `
    ...
    .fx-bg-img{position:absolute;inset:-10% 0 -10% 0;width:100%;height:120%;object-fit:cover;filter:brightness(0.45);opacity:0;will-change:transform,opacity}
    .fx-bg-overlay{position:absolute;inset:0;background:var(--fx-overlay)}
    .fx-header{grid-column:1/13;align-self:start;padding-top:5vh;font-size:10px;text-align:center;color:var(--fx-text);letter-spacing:0.6em;text-transform:uppercase;font-family:'Jost',sans-serif;opacity:0.5}
    .fx-content{grid-column:1/13;position:absolute;inset:0;display:grid;grid-template-columns:1fr 1.4fr 1fr;align-items:center;height:100%;padding:0 var(--fx-grid-px)}
    .fx-left,.fx-right{height:65vh;overflow:hidden;display:grid;align-content:center}
    .fx-left{justify-items:start}.fx-right{justify-items:end}
    .fx-track{will-change:transform}
    .fx-item{color:var(--fx-text);font-weight:500;letter-spacing:0.12em;line-height:1.1;margin:calc(var(--fx-row-gap)/2) 0;opacity:0.35;transition:opacity 0.3s,transform 0.3s;position:relative;font-size:clamp(0.75rem,1.4vw,1.1rem);cursor:pointer;text-transform:uppercase;font-family:'Jost',sans-serif}
    .fx-left-item.active,.fx-right-item.active{opacity:1;color:#d4af58}
    ...
    .fx-left-item.active::before{content:"";position:absolute;top:50%;left:0;transform:translateY(-50%);width:4px;height:4px;background:#d4af58;border-radius:50%}
    .fx-right-item.active::after{content:"";position:absolute;top:50%;right:0;transform:translateY(-50%);width:4px;height:4px;background:#d4af58;border-radius:50%}
    .fx-center{display:grid;place-items:center;text-align:center;height:65vh;overflow:hidden}
    .fx-featured{position:absolute;opacity:0;visibility:hidden;pointer-events:none}
    .fx-featured.active{opacity:1;visibility:visible;pointer-events:auto}
    .fx-featured-title{margin:0;color:var(--fx-text);font-weight:400;font-style:italic;font-size:clamp(2rem,5.5vw,4.5rem);font-family:'Cormorant Garamond',Georgia,serif;letter-spacing:0.02em}
    ...
    .fx-footer{grid-column:1/13;align-self:end;padding-bottom:4vh;text-align:center}
    .fx-progress{width:160px;height:1px;margin:1rem auto 0;background:rgba(212,175,88,0.15);position:relative}
    .fx-progress-fill{position:absolute;inset:0 auto 0 0;width:0%;background:#d4af58;height:100%;transition:width 0.3s}
    .fx-progress-numbers{position:absolute;inset:auto 0 100% 0;display:flex;justify-content:space-between;font-size:0.7rem;color:rgba(212,175,88,0.4);font-family:'Jost',sans-serif;letter-spacing:0.25em;padding-bottom:6px}
    .fx-end{height:10vh}
    @media(max-width:768px){.fx-content{grid-template-columns:1fr;place-items:center}.fx-left,.fx-right{height:auto;justify-items:center}.fx-track{transform:none!important}}
  `;

  return (
    <div
      ref={(node)=>{(rootRef as any).current=node;if(typeof ref==="function")ref(node);else if(ref)(ref as React.MutableRefObject<HTMLDivElement|null>).current=node;}}
              ...
              {sections.map((s,i)=>(
                <div className="fx-bg" key={s.id??i}>
                  {s.renderBackground?s.renderBackground(index===i,lastIndexRef.current===i):(
                    <><img ref={(el)=>{if(el)bgRefs.current[i]=el;}} src={s.background} alt="" className="fx-bg-img"/><div className="fx-bg-overlay"/></>
                  )}
                </div>
              ))}
            </div>
              ...
              <div className="fx-content">
                <div className="fx-left" role="list">
                  <div className="fx-track" ref={leftTrackRef}>
                    {sections.map((s,i)=>(
                      <div key={"L-"+(s.id??i)} className={"fx-item fx-left-item "+(i===index?"active":"")}
                  ...
                  {sections.map((s,sIdx)=>{
                    ...
                    return(
                      <div key={"C-"+(s.id??sIdx)} className={"fx-featured "+(sIdx===index?"active":"")}>
                        <h3 className="fx-featured-title">{isStr?splitWords(s.title as string):s.title}</h3>
                ...
                <div className="fx-right" role="list">
                  <div className="fx-track" ref={rightTrackRef}>
                    {sections.map((s,i)=>(
                      <div key={"R-"+(s.id??i)} className={"fx-item fx-right-item "+(i===index?"active":"")}
